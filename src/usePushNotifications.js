...
export default function usePushNotifications() {
  ...
  // to manage the user consent: Notification.permission is a
  // JavaScript native function that return the current state of the permission
  ...
  useEffect(() => {
    if (!pushServerSubscriptionId && userSubscription) {
      setLoading(true);
      setError(false);
      http
  ...
  }, [pushServerSubscriptionId, userSubscription]);
  ...
  const onClickSendNotification = async (id) => {
    console.log("YOU TOUCHED MY TALALA", id);
    setLoading(true);
    setError(false);
    await http.get(`/api/subscription/${id}`).catch((err) => {
      setLoading(false);
      setError(err);
    });
    setLoading(false);
